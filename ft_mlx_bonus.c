...
{	
	if (data->game->map[y][x] == '1')
		...
		data->game->map[data->game->p_y][data->game->p_x] == 'V')
		ft_dead_player(x, y, data);
...
int	ft_key(int key, t_data *data)
{
	...
	x = data->game->p_x;
	y = data->game->p_y;
	if (data->game->items == 0 && data->game->end == 0)
		ft_put_image(EXIT_OPEN, data->game->exit[1], data->game->exit[0], data);
	ft_key_other(key, data);
	if ((key == W || key == UP || key == A || key == LEFT || key == S
			|| key == DOWN || key == D || key == RIGHT) && data->game->end == 0)
		ft_enemy(data);
	if ((key == W || key == UP) && data->game->end == 0)
		ft_move(x, y - 1, data);
	else if ((key == A || key == LEFT) && data->game->end == 0)
		ft_move(x - 1, y, data);
	else if ((key == S || key == DOWN) && data->game->end == 0)
		ft_move(x, y + 1, data);
	else if ((key == D || key == RIGHT) && data->game->end == 0)
		ft_move(x + 1, y, data);
	return (0);
}
